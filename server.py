"""GeoQuiz MCP server for VWorld satellite-based map quizzes."""
...
def create_map_quiz(
    ...
    try:
        ...
        print(f"✅ 검증 성공: {address}")
        
    except Exception as e:
        error_msg = f"❌ 위치 검증 실패: {str(e)}\n올바른 위도/경도로 다시 요청해주세요."
        print(error_msg)
    ...
    location_data = {
        "condition": condition,
        "address": address,  # 검증된 주소를 정답으로 저장
        "quiz_type": quiz_type,
        "lat": lat,
        "lon": lon,
        "zoom": zoom,
